...
void FImGuiContextManager::BuildFontAtlas()
{
	if (!FontAtlas.IsBuilt())
	{
		...
		bool bUsingCustomFont = false;

		FString FontDirectory =  FPaths::Combine(FPaths::ProjectDir(), TEXT("ExternalData/Font"));
		UE_LOG(LogTemp, Log, TEXT("ImGui Font Directory = %s"), *FontDirectory);

		TArray<FString> Files;
		IFileManager::Get().FindFiles(Files, *FontDirectory, TEXT(".ttf"));

		if(Files.Num() > 0)
		{
			FString FontFullPath = FPaths::Combine(FontDirectory, Files[0]);
			bUsingCustomFont = FontAtlas.AddFontFromFileTTF(TCHAR_TO_ANSI(*FontFullPath),
		13.f * DPIScale, &FontConfig, FontAtlas.GetGlyphRangesChineseSimplifiedCommon()) != nullptr;
		}
		else
		{
			UE_LOG(LogTemp, Log, TEXT("missing imgui font file."));
		}
